...
## 8. Phase 6 — 구조화 응답 API와 SSE

### 8.1 목표

Phase 6의 목표는 상담 결과를 화면이 직접 가공하기 좋은 구조로 제공하고, 긴 답변은 생성되는 즉시 표시하는 것이다.

- 일반 API는 답변, 출처, Tool 사용 여부를 분리한 JSON으로 반환한다.
- 스트리밍 API는 답변 조각을 SSE `token` 이벤트로 전달한다.
- 스트림의 마지막에는 SSE `sources` 이벤트를 보내 출처와 Tool 사용 여부를 확정한다.
- 인증 사용자와 Phase 5의 세션 ID를 그대로 사용해 메모리·도구 격리를 유지한다.

### 8.2 적용 전과 후

| 구분 | 적용 전 | 적용 후 |
|---|---|---|
| 응답 형식 | 서비스 내부의 답변 객체만 존재 | `answer`, `sources`, `toolUsed` 구조화 JSON 제공 |
| 긴 답변 표시 | 전체 생성이 끝날 때까지 기다림 | 생성 조각을 `token` 이벤트로 즉시 표시 |
| 스트리밍 출처 | 별도 전달 규칙 없음 | 마지막 `sources` 이벤트로 출처 목록 확정 |
| 호출 경계 | Controller 골격만 존재 | `/api/chat`, `/api/chat/stream` API 제공 |
| 사용자 식별 | 서비스 호출자가 직접 값 전달 | 인증된 `Principal`의 사용자 ID 사용 |
| 입력 검증 | 서비스의 필수값 검사 | 요청 DTO에서 질문 길이와 sessionId 형식 선검증 |

### 8.3 동기 구조화 API

`POST /api/chat`은 답변 생성이 끝난 뒤 다음 구조를 반환한다.

```json
{
  "answer": "단순 변심은 수령 후 7일 이내에 신청할 수 있습니다.",
  "sources": [
    {
      "document": "return-policy.md",
      "title": "반품 규정",
...
`sources`는 Phase 3에서 실제 검색된 문서만 사용하고, `toolUsed`는 Phase 4의 Tool이 실행되었는지를 나타낸다. 화면은 문자열을 다시 분석하지 않고 출처 영역과 Tool 사용 표시를 구성할 수 있다.

### 8.4 SSE 스트리밍 API

`POST /api/chat/stream`은 `text/event-stream;charset=UTF-8`로 응답한다. 이벤트 순서는 다음과 같다.
...
event: sources
data: {"sources":[...],"toolUsed":false}
```

클라이언트는 `token`의 data를 순서대로 이어 붙여 답변을 표시하고, 마지막 `sources` 이벤트를 받으면 출처 UI를 완성한다. UTF-8을 명시해 한글 토큰이 클라이언트마다 다르게 해석되는 문제를 막았다.

### 8.5 서비스의 스트리밍 안전선

스트리밍에서도 Phase 3의 “근거 없는 답변은 내보내지 않는다”는 규칙을 유지한다.

1. 각 `ChatClientResponse`에서 검색 문서와 Tool 실행 여부를 확인한다.
2. 문서 근거나 Tool 실행이 확인된 응답 조각만 `token`으로 변환한다.
3. 끝까지 신뢰 가능한 근거가 없으면 모델 출력 대신 고정된 근거 부족 문구를 보낸다.
4. 스트림 종료 직전에 출처 목록과 Tool 사용 여부를 `sources` 이벤트로 보낸다.

따라서 동기 API와 스트리밍 API가 서로 다른 안전 정책을 갖지 않는다.

### 8.6 요청·인증·타임아웃

- `question`: 공백일 수 없으며 최대 2,000자다.
- `sessionId`: 공백일 수 없고 영문자, 숫자, `_`, `-`만 허용한다.
- `userId`: 요청 본문을 신뢰하지 않고 인증된 `Principal`에서 가져온다.
- `tenantId`: 현재는 `X-Tenant-Id` 헤더를 받으며 기본값은 `skala`다.
- 스트림 제한 시간: 응답이 무한히 열린 채 남지 않도록 60초 타임아웃을 둔다.

운영 환경에서는 `X-Tenant-Id`를 임의 헤더로 신뢰하지 않고 인증 토큰 또는 서버 측 테넌트 매핑에서 가져와야 한다.

### 8.7 검증

Controller 테스트에서 다음을 확인한다.

- 인증 사용자로 동기 API를 호출하면 구조화된 `answer`, `sources`, `toolUsed`가 반환되는지 확인
- SSE 응답이 `token` 이벤트를 먼저 보내고 `sources` 이벤트를 마지막에 보내는지 확인
- SSE 한글 토큰이 UTF-8로 유지되는지 확인
- 허용하지 않은 문자가 포함된 sessionId 요청을 거부하는지 확인
- 기존 Phase 1~5 테스트가 계속 통과하는지 확인

### 8.8 주요 산출물

| 파일 | 역할 |
|---|---|
| `web/ChatController.java` | 구조화 REST 및 SSE 엔드포인트 |
| `chat/AnswerDto.java` | 답변·출처·Tool 사용 여부 응답 계약 |
| `chat/StreamEvent.java` | 내부 스트리밍 이벤트와 최종 요약 계약 |
| `chat/HelpDeskService.java` | ChatClient 스트림을 안전한 token/sources 이벤트로 변환 |
| `web/ChatControllerTests.java` | JSON, SSE 순서, UTF-8, 요청 검증 테스트 |
...
- 클라이언트 연결 해제 시 모델 요청 취소와 비용 절감 여부 검증
- 스트림 중간 오류를 표현할 명시적인 `error` 이벤트 계약
- 프록시와 로드밸런서의 SSE 버퍼링·타임아웃 설정
- 실제 OpenAI 스트림을 사용한 E2E 및 첫 토큰 지연 측정
- `X-Tenant-Id`를 인증 정보 기반 테넌트 식별로 교체

## 9. 종합 검증 — 다섯 흐름

### 9.1 목적

종합 검증은 Phase별 클래스가 따로 동작하는지만 확인하지 않고, HelpDesk가 실제 상담 흐름에서 근거·맥락·행동·안전·가용성을 함께 유지하는지 확인한다. 모델의 자연어 표현은 매번 조금 달라질 수 있으므로 완성 문장 전체가 아니라 출처, Tool 호출, 승인 상태와 필수 문구를 검증 기준으로 삼는다.

### 9.2 골든 시나리오

| 흐름 | 사용자 발화 | 기대 결과 | 검증 대상 |
|---|---|---|---|
| 규정 | “반품 규정 알려줘” | RAG 답변과 문서 출처 | 근거·출처 |
| 후속 | “반품 규정 알려줘” → “내 주문 12345는?” → “그럼 그건 반품돼요?” | 같은 세션의 규정 문맥과 주문 Tool 결과를 함께 사용 | Memory + RAG + Tool |
| 행동 | “주문 12345를 단순 변심으로 교환 접수해줘” | 티켓 번호, `PENDING`, 승인 후 처리 안내 | 쓰기 Tool 승인 게이트 |
| 안전 | user2가 user1의 주문 12345 조회 | 주문 존재 여부를 노출하지 않고 동일한 미조회 응답 | 소유권 검증·정보 비노출 |
| 폴백 | 문서에 없는 규정 질문 | “제공된 문서에서 확인되지 않습니다” | 환각 방지·가용성 |

시나리오는 `GoldenSet`에 코드로 관리한다. 각 항목은 userId와 sessionId, 순서가 있는 사용자 발화, 출처·Tool·승인 필요 여부 및 필수 응답 신호를 가진다.

### 9.3 자동 검증 범위

현재 자동 테스트는 외부 OpenAI 호출 없이 다음 불변 조건을 검증한다.

- 골든 세트가 다섯 흐름을 하나씩 모두 포함하는지 확인
- 후속 시나리오의 세 발화가 한 세션 안에서 정해진 순서를 유지하는지 확인
- 후속 시나리오가 문서 출처와 주문 Tool을 모두 요구하는지 확인
- 행동 시나리오가 `PENDING`과 승인 후 처리 문구를 요구하는지 확인
- 안전 시나리오가 타인 주문의 존재 여부를 노출하지 않는 문구를 요구하는지 확인
- 폴백 시나리오가 근거 부족 고정 문구를 요구하는지 확인

기존 단위·통합 테스트는 실제 구현의 다음 경계를 담당한다.

| 검증 경계 | 테스트 |
|---|---|
| RAG 출처와 무근거 차단 | `HelpDeskServiceTests` |
| JDBC 메모리 복원과 세션 격리 | `ChatMemoryPersistenceTests`, `ConversationIdFactoryTests` |
| 주문 소유권과 정보 비노출 | `OrderToolsTests` |
| 티켓 PENDING 승인 게이트 | `TicketToolsTests` |
| 구조화 JSON과 SSE 순서 | `ChatControllerTests` |
| 다섯 흐름 정의의 완전성 | `GoldenSetTests` |

### 9.4 실제 모델 E2E에서 추가 확인할 내용

골든 세트의 구조 검증만으로 모델이 올바른 Tool을 선택한다고 증명할 수는 없다. OpenAI API와 PGvector를 연결한 E2E 환경에서는 다음을 추가 측정한다.

- 규정 질문에서 주문·티켓 Tool을 잘못 호출하지 않는지
- “그건”이 같은 sessionId의 주문 12345와 반품 규정을 가리키는지
- 행동 요청에서 티켓 Tool을 정확히 한 번만 호출하는지
- 타인 주문 요청에서 응답이나 로그에 주문 정보가 섞이지 않는지
- 문서 근거가 없을 때 모델의 사전학습 지식을 답변으로 내보내지 않는지
- 주 모델 장애 시 폴백 모델로 응답이 지속되는지

### 9.5 주요 산출물

| 파일 | 역할 |
|---|---|
| `eval/GoldenSet.java` | 다섯 대표 상담 흐름과 의미 기반 기대 결과 |
| `eval/GoldenSetTests.java` | 시나리오 완전성, 멀티턴, 승인·안전·폴백 계약 검증 |
