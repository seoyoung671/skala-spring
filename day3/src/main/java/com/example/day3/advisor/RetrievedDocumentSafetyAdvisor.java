...
public class RetrievedDocumentSafetyAdvisor implements CallAdvisor {
    ...
    public ChatClientResponse adviseCall(ChatClientRequest request, CallAdvisorChain chain) {
        ...
        if (value instanceof List<?> values) {
            Document unsafeDocument = values.stream()
                    .filter(Document.class::isInstance)
                    .map(Document.class::cast)
                    .filter(document -> injectionDetector.containsInjection(document.getText()))
                    .findFirst()
                    .orElse(null);
            if (unsafeDocument != null) {
                audit.error("event=RAG_DOCUMENT_BLOCKED source={}",
                        unsafeDocument.getMetadata().getOrDefault("source", "unknown"));
                return SafetyAdvisor.blockedResponse(request, "indirect_prompt_injection", FAILURE_RESPONSE);
            }
        }
